...
- **Main** — removed the Notes section widget. The "New Note" quick-capture button now writes to the new `notes` table and the note appears on the Notes Dashboard.
...
- **PersonPage linked tasks and projects — filter out completed items** — PersonPage Tasks and Projects sections now only show active items (exclude status='done'/'archived' for tasks, status='completed' for projects, and any archived_at items). Keeps the view focused on work in progress.
...
### Changed
- **"Daily" renamed to "Dashboard"** in the sidebar nav; icon swapped from Sun to LayoutDashboard.
