...
### Changed
- **All emojis swapped for Lucide icons across the app** — sidebar nav (Sun / FolderKanban / Zap / Users / Target / NotebookText / Settings / LogOut), task & project status pills (Inbox / Zap / Layers / Calendar / Hourglass / Sparkles / Check), all 8 habit rows (Pill / Moon / PenLine / Brain / Wind / StretchHorizontal / Heart / Code), TaskRow + ProjectRow indicators (Star highlight, Folder/FolderOpen area, AlertCircle deadline, AlertTriangle overdue, Check select), TaskPage action buttons (Archive, Star, Trash2, Calendar, AlertCircle, User), Modal close (X), TaskCompletionModal (Archive / Star / Clock), ChallengeSection reveal (ChevronDown/Right), Reviews under-construction (HardHat), Login magic-link success (MailOpen). Decorative inline emojis (📋 🎉 🚧 📬 ⚠ 🔮 🔴 ⭐ 📁 🗑 ⧉ 👤 📅 📂 ✓ ✗ ✕ ☑) stripped wherever they were just text dressing. A new `<LucideIcon name="Zap" size={11}/>` lookup component renders Lucide icons by name string — used by `StatusPill`, `EnergyBadge`, `HabitsSection`, `Habits`, `HabitPage`, `EmptyState`, and all four Settings list previews. Constants files (`TASK_STATUSES`, `HABITS`) now store Lucide name strings instead of emoji glyphs. The Settings → Energy Levels / Areas icon inputs are now Lucide-name text fields (placeholders like `Brain`, `Dumbbell`, `Home` with live preview) instead of emoji-only inputs; the `energy_levels.icon` column was migrated in place (💪→Dumbbell, 🧠→Brain, 💻→Monitor, 🌿→Leaf, 💼→Briefcase, 🛒→ShoppingCart). The `people.icon` avatar-emoji fallback was removed from `AvatarCircle` (no more emoji-on-avatar — falls through to initials), along with the corresponding "Avatar Icon (emoji)" input on PersonPage.
